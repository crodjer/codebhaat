...
from profiles.create_excel import ExcelResponse
    ...
    **Template:**
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            profile_obj.user.first_name = profile_obj.name
            #profile.user.save()
            profile_obj.save()
...
def user_list_excel(request):
    ...
    data_list = [
        ["Full Name", "Username", "Gender", "Email Address", "College", "State", "Year", "Contact Number", "Home Address"],
        ]

    for u in users:
        try:
            data_list.append(
                [u.get_profile().name, u.username, u.get_profile().get_gender_display(), u.email, u.get_profile().get_college_display(), u.get_profile().get_state_display(), u.get_profile().get_year_display(), u.get_profile().contact, u.get_profile().home_address]
                )
            
        except:
            data_list.append(
                             ["No data", u.username, "No data", u.email, "No data", "No data", "No data", "No data", "No data"]
                             )
